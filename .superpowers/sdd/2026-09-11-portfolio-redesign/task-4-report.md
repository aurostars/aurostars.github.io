...
- collage 使用 `role="group"` 和可访问名称“个人项目界面预览”，每张图片沿用真实媒体替代文本并保持独立可访问。
- 未使用渐变、玻璃拟态或图片叠加装饰标签。
...
6. 可访问性审查后先补充命名分组与三张独立命名图片的行为测试，验证测试因找不到 `group` 得到预期 RED，再将 collage 从 `role="img"` 修正为 `role="group"`。

## 验证结果

- 目标测试：通过，4 tests passed。
- ESLint：通过。
- TypeScript：通过。
